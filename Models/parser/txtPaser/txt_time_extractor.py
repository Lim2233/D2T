...
支持多种常见日期格式，自动提取第一个和最后一个出现的日期作为开始和结束时间。
可作为模块导入，也可直接通过命令行运行。
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="从 TXT 文件中提取时间范围并输出为 JSON 文件"
    )
    parser.add_argument("input", help="输入的 TXT 文件路径")
    parser.add_argument("-o", "--output-dir", default=".",
                        help="输出目录，默认为当前目录")
    parser.add_argument("-n", "--name", help="输出文件名（不含扩展名），默认使用输入文件名")
    ...
    try:
        output_path = process_txt_file(args.input, args.output_dir, args.name)
        print(f"✅ 成功提取时间范围，结果已保存至：{output_path}")
    except Exception as e:
        logger.exception("处理过程中发生未捕获的异常")
